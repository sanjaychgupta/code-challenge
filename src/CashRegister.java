...
public class CashRegister {
    public static void main(String[] args) {
        String line = "45;50";
        System.out.print(CashRegister.getCashChange(line));
    }

    private static String getCashChange(String line) {
        String[] fields = line.split(";", 2);
        float purchasePrice = Float.parseFloat(fields[0]);
        float cashReceived = Float.parseFloat(fields[1]);
        float cashBack = cashReceived - purchasePrice;
        if (cashBack < 0) {
            return ("ERROR");
        } else if (cashBack == 0) {
            return ("ZERO");
